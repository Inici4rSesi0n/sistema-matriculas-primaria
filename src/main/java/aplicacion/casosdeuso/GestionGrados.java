...
public class GestionGrados {
    ...
    public void crearGrado(String nombre, String nivel) {
        ...
        if (nivel == null || nivel.isBlank()) {
            throw new IllegalArgumentException("El nivel del grado no puede estar vacío.");
        }
        if (repo.buscarPorNombre(nombre).isPresent()) {
            throw new IllegalArgumentException("Ya existe un grado con ese nombre.");
        }
        repo.agregar(new Grado(nombre, nivel));
    }

    public Grado buscarPorNombre(String nombre) {
